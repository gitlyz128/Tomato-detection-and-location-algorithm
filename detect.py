            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if (x <= 672):
                        ...
                        gray_left = cv2.cvtColor(iml, cv2.COLOR_BGR2GRAY)
                        gray_right = cv2.cvtColor(imr, cv2.COLOR_BGR2GRAY)

                        num_disparities =10
                        block_size = 15
                        num_disparities = num_disparities * 16
                        stereo_sgbm.setNumDisparities(num_disparities)
                        stereo_sgbm.setBlockSize(block_size)
                        ...
                        cv2.imshow( "disp",disparity_sgbm_norm)
                        ...
                        text_z = "z:%.1fcm" % (points_3d[int(y+10), int(x), 2] / 10)
                        ...
                        put_tomato_location.append(text_x)
                        put_tomato_location.append(text_y)
                        put_tomato_location.append(text_z)
                    ...
                    ####################################################
                        ...
                        if stopmove == 1:
                            ...
                            if x != 0 and y != 0:
                                ...
                                # TomatoXYZ = Tomatox + Tomatoy + Tomatoz
                                tomato_location_str = ''.join(put_tomato_location)
                                tcp_client.send(tomato_location_str.encode("utf-8"))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'best192320half.engine', help='model path or triton URL')
...
if __name__ == '__main__':
    ...
    # print(123233)
    t2.start()
    t1 = threading.Thread(target=mainn(opt))
